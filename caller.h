...
class caller
  ...
  public:
    //! 不定参调用
    unsigned __int64 operator()(...) const;
    //! 远程调用
    unsigned __int64 request(...) const;
    //! 允许临时改变this指针
    caller& changethis(void* const lp_this);
    //! 重载操作允许在一个语句内完成this修改并调用
    caller& operator[](void* const lp_this);
  ...
  public:
    ...
    */
    static void mov_call(unsigned char* const newmem);
