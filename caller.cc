...
__declspec(naked) unsigned __int64 fakecall(const caller* nowthis, void* argv)
  {
  __asm
    {
    push    0x01230814
...
static unsigned __int64(*g_lp_fakecall)(const caller* nowthis, void* argv) =
(unsigned __int64(*)(const caller *, void *))&fakecall;  //指向伪call，用于伪call代码的转移

unsigned __int64 caller::operator()(...) const
  {
...
void caller::mov_call(unsigned char* const newmem)
  {
  if(newmem == NULL)
    {
    g_lp_fakecall = (unsigned __int64(*)(const caller *, void *))&fakecall;
    return;
    }

  const unsigned char* lp = (const unsigned char*)&fakecall;
  size_t len = 0;
  while(lp[len++]);
  Hookit(newmem, lp, len);

  g_lp_fakecall = (unsigned __int64(*)(const caller *, void *))newmem;
  }
...
struct caller_request
  ...
  unsigned __int64  return_value;     //申请结果
...
//发起远程申请
unsigned __int64 fakerequest(const caller* nowthis, void* argv)
  {
  xevent xe;
...
unsigned __int64 caller::request(...) const
  {
  void** ap = (void**)_AddressOfReturnAddress();
  ++ap;
#ifndef _WIN64
